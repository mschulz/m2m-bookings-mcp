...
def backfill_data(today, use_db=True):
    ...
    recurring_customer_count = (
        booking_dao.recurring_current()
    )  # this include nett since midnight last night
    recurring_customer_count -= get_today_nett(today)

    while start_created >= begin_datetime_utc:
        today_gain, today_loss = booking_dao.gain_cancelled_in_range_new(
            start_created, end_created
        )
        ...
        print(
            f"Day gains = {booking_dao.get_gain_in_date_range_list(start_created, end_created)}"
        )
        print(
            f"Day cancels = {booking_dao.get_cancelled_in_date_range_list(start_created, end_created)}"
        )
        ...
        if use_db:
            ...
            print(
                ...
                nett_for_day,
                recurring_customer_count,
                is_saturday(day_date),
                is_end_of_month(day_date),
            )
