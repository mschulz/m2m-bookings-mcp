"""Booking model with all columns and webhook import logic."""
...
class BookingBase(SQLModel):
    ...
    def update_from_cancellation(self, b: dict):
        """Apply cancellation-specific fields from webhook data."""
        bid = b.get("booking_id")
        self.booking_id = safe_int(b["booking_id"])
        self.updated_at = parse_datetime(b.get("updated_at"))
        self.cancellation_type = truncate_field(b.get("cancellation_type"), 64, "cancellation_type", bid)
        self.cancelled_by = truncate_field(b.get("cancelled_by"), 64, "cancelled_by", bid)
        self.cancellation_date = parse_date(b.get("cancellation_date"))
        self.cancellation_datetime = b.get("_cancellation_datetime")
        self.cancellation_reason = b.get("cancellation_reason")
        if b.get("cancellation_fee") is not None and str(b.get("cancellation_fee")) != "":
            self.cancellation_fee = dollar_string_to_int(str(b["cancellation_fee"]))
        self.booking_status = truncate_field(b.get("booking_status"), 64, "booking_status", bid)
        self.is_first_recurring = string_to_boolean(b["is_first_recurring"]) if b.get("is_first_recurring") is not None else False
        if self.is_first_recurring:
            self.was_first_recurring = True
        self.is_new_customer = string_to_boolean(b["is_new_customer"]) if b.get("is_new_customer") is not None else False
        if self.is_new_customer:
            self.was_new_customer = True
        self.postcode = check_postcode(b, "booking_id", b["booking_id"])
        if b.get("location"):
            self.location = truncate_field(b["location"], 64, "location", bid)

        if "custom_fields" in b:
            process_custom_fields(self, b["custom_fields"], bid)
