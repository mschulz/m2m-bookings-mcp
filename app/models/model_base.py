...
class BookingBase(db.Model):
    ...
    duration = db.Column(db.String(32), index=False, unique=False)
    ...
    _next_booking_date = db.Column(
        db.DateTime(timezone=True), index=False, unique=False
    )
    ...
    _cancellation_datetime = db.Column(
        db.DateTime(timezone=True), index=False, unique=False
    )  ########### NEW #########
    ...
    was_first_recurring = db.Column(
        db.Boolean, index=False, unique=False, default=False
    )
        ...
        if val:
            try:
                ...
                current_app.logger.error(
                    f'cancellation_date error: "{val}" leads to error: {e}'
                )
    ...
    def price_adjustment(self, val):
        ...
        if val is not None and len(val) > 0:
            try:
                ...
                current_app.logger.error(
                    f'price_adjustment error val="{val}": error_message:{e}'
                )
    ...
    def sms_notifications_enabled(self, val):
        if val is not None:
            self._sms_notifications_enabled = (
                string_to_boolean(val) if val is not None else False
            )
    ...
    def import_dict(d, b):
        ...
        d.service_category = b.get(
            "service_category", current_app.config["SERVICE_CATEGORY_DEFAULT"]
        )
        if "service" in b:
            if len(b["service"]) > 128:
                current_app.logger.error(
                    f"booking_id: {b['id']}:: truncating service field to 128 characters {b['service']}"
                )
...
def process_custom_fields(d, b_cf):
    ## How did you find Maid2Match
    ...
    # Invoice Reference (e.g. NDIS #) (262)
    CUSTOM_INVOICE_REFERENCE_EXTRA = current_app.config[
        "CUSTOM_INVOICE_REFERENCE_EXTRA"
    ]
...
def dollar_string_to_int(val):
    ...
    ):  # No idea why I need to do this check for a string name
        return 0
    if isinstance(val, str):
