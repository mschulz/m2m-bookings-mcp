...
class BookingDAO(BaseDAO):
    ...
    def all_bookings_by_created_at_dates(self, from_date_str, to_date_str):
        return (
            db.session.query(self.model)
    ...
    def get_cancelled_in_date_range(self, date_start, date_end):
        ...
        cancelled_in_date_range = [item.booking_id for item in items.all()]
        print(
            f"get_cancelled_in_date_range({len(cancelled_in_date_range)}): {cancelled_in_date_range}"
        )
    ...
    def get_bookings_missing_locations(self):
        return db.session.query(self.model).filter(self.model.location is None).all()
